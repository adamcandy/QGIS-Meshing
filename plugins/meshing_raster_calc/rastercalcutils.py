...
def lastUsedDir():
  settings = QSettings()
  return settings.value( "/Meshing Raster Calc/lastDir", QVariant( "" ) ).toString()

def setLastUsedDir( lastDir ):
  path = QFileInfo( lastDir ).absolutePath()
  settings = QSettings()
  settings.setValue( "/Meshing Raster Calc/lastDir", QVariant( path ) )

def addToCanvas():
  settings = QSettings()
  return settings.value( "/Meshing Raster Calc/addLayer", QVariant( False ) ).toInt()[ 0 ]

def setAddToCanvas( state ):
  settings = QSettings()
  settings.setValue( "/Meshing Raster Calc/addLayer", QVariant( state ) )
