...
fname = "test_annulus_BSplines_Compound_lines"
command = 	"--line BY -g "+test+"/test_annulus_BSplines_Compound_lines/test_annulus_BSplines_Compound_lines.geo "+support_file_path+"/annulus.shp --mesh --mval 10"
...
def test_annulus_bn_geo():
	curr_geo_file = os.path.dirname(os.path.realpath(__file__)) + "/output/" + fname + "/" + fname + ".geo"

	assert geo_files_test(curr_geo_file),"%s does not match the model answer" % (ntpath.basename(curr_geo_file).rstrip())
